...
export function setup(config, cb) {
  ...
  return table.indexList().run(conn).then(indexes => {
      ...
      return table.indexCreate('authorTags',
        (post) => post('tags').map(tag => [ post('author')]),
        { multi: true }).run(conn);
...
export function filterOnly(conn, author, tag) {
...
export function getAllAndFilter(conn, author, tag) {
  return r.table('posts')
...
export function mapped(conn, author, tag) {  
  return r.table('posts')
